...
function getSumUPC(arrUpcReverse:string[]): number{
let evenNumber = 0
let oddNumber = 0
for (const [index, numberChar] of arrUpcReverse.entries()) {
    
    switch((index+1)%2){
        case 1: oddNumber += Number(numberChar)
                break;
        case 0: evenNumber +=Number(numberChar)
                break;
    }
}
...
function validateUPC(resultNumber:number, firstCharNumber: number): boolean{

    const mod = resultNumber%10
    switch(mod){
        case 0 : return (0 ===firstCharNumber)
                
        default :  console.log((10 -mod))
